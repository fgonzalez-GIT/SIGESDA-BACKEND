...
export class ReciboService {
  ...
  async createRecibo(data: CreateReciboDto): Promise<Recibo> {
    ...
    // Create receipt (numero auto-generated by PostgreSQL sequence)
    const recibo = await this.reciboRepository.create(data);

    logger.info(`Recibo creado: ${recibo.numero} - ${data.tipo} - $${data.importe} (ID: ${recibo.id})`);
  ...
  async createBulkRecibos(data: CreateBulkRecibosDto): Promise<{ count: number; errors: string[] }> {
    const errors: string[] = [];
    const validRecibos: CreateReciboDto[] = [];
    ...
    for (const recibo of data.recibos) {
      try {
        ...
        // Add to valid list (numero will be auto-generated by database)
