 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Service layer for generating comprehensive reports and analytics
 *
 * @module ReportesCuotaService
...
import {
  DashboardCuotasDto,
  ReportePorCategoriaDto,
  AnalisisDescuentosDto,
  ReporteExencionesDto,
  ReporteComparativoDto,
  EstadisticasRecaudacionDto
} from '@/dto/reportes-cuota.dto';

export class ReportesCuotaService {
  private get prisma() {
    return prisma;
  }
   ...
   */
  async getDashboard(params?: DashboardCuotasDto): Promise<any> {
    const currentDate = new Date();
    const mes = params?.mes || currentDate.getMonth() + 1;
    const anio = params?.anio || currentDate.getFullYear();

    logger.info(`[REPORTES] Generando dashboard para ${mes}/${anio}`);

    // Get cuotas for the period
   ...
   */
  async getReportePorCategoria(params: ReportePorCategoriaDto): Promise<any> {
    logger.info(`[REPORTES] Reporte por categoría - ${params.mes}/${params.anio}`);
    ...
    // Group by category
    const categorias = new Map<number, any>();
  ...
  /**
   * Analyze all types of discounts applied
   */
  async getAnalisisDescuentos(params: AnalisisDescuentosDto): Promise<any> {
    logger.info(`[REPORTES] Análisis de descuentos - ${params.mes}/${params.anio}`);
    ...
    if (params.tipoDescuento === 'reglas' || params.tipoDescuento === 'todos') {
      ...
      analisis.reglasAutomaticas.sociosAfectados = new Set(
        aplicaciones.map(a => a.cuota.recibo.receptorId)
      ).size;
      analisis.reglasAutomaticas.montoTotalDescuento = aplicaciones.reduce(
        (sum, a) => sum + Number(a.montoDescuento),
        0
      );
    ...
    if (params.tipoDescuento === 'exenciones' || params.tipoDescuento === 'todos') {
      ...
      for (const exencion of exenciones) {
        // Estimate exemption amount (would need actual cuota data)
  ...
  /**
   * Generate report of exemptions
   */
  async getReporteExenciones(params: ReporteExencionesDto): Promise<any> {
    logger.info(`[REPORTES] Reporte de exenciones - Estado: ${params.estado}`);
    ...
    if (params.categoriaId) {
      exencionesFiltradas = exenciones.filter(e =>
        e.persona.tipos.some(t => t.categoriaId === params.categoriaId)
      );
    }
   ...
   */
  async getReporteComparativo(params: ReporteComparativoDto): Promise<any> {
    logger.info(
      `[REPORTES] Reporte comparativo - ` +
    ...
    const variaciones = {
      totalCuotas: {
        ...
        porcentaje: this.calculatePercentageChange(
          periodoInicio.metricas.totalCuotas,
          periodoFin.metricas.totalCuotas
        )
      },
      totalRecaudado: {
        ...
        porcentaje: this.calculatePercentageChange(
          periodoInicio.metricas.totalRecaudado,
          periodoFin.metricas.totalRecaudado
        )
      },
      tasaPago: {
   ...
   */
  async getEstadisticasRecaudacion(params?: EstadisticasRecaudacionDto): Promise<any> {
    logger.info(`[REPORTES] Estadísticas de recaudación`);
  ...
  private calculatePercentageChange(old: number, nuevo: number): number {
    if (old === 0) return nuevo > 0 ? 100 : 0;
