...
class CuotaService {
    ...
    async calcularMontoCuota(data) {
        ...
        try {
            ...
            logger_1.logger.warn(`No se pudo obtener configuración de precio para ${data.categoria}, usando valor por defecto`);
        }
        ...
        if (data.aplicarDescuentos) {
            const descuentosCalculados = await this.calcularDescuentos(data.categoria, montoBase, data.socioId);
    ...
    async calcularDescuentos(categoria, montoBase, socioId) {
        const descuentos = [];
        let total = 0;
        if (categoria === client_1.CategoriaSocio.ESTUDIANTE) {
        ...
        if (categoria === client_1.CategoriaSocio.JUBILADO) {
